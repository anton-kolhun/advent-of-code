...
public class Day20Part2 {
    ...
    public static void task1() {
        ...
        String rxDependsOn = "zr";
        Set<String> rxDependsOnConjDependencies = conjDeps.get(rxDependsOn).keySet();
        Map<String, Integer> depsMatchingCyclesDiffs = new HashMap<>();
        Map<String, Integer> matchingCycles = new HashMap<>();
        ...
        while (shouldContinue) {
            ...
            while (!pulses.isEmpty()) {
                ...
                if (pulseInfo.sentTo.equals("rx")) {
                    for (String cursorConjDep : rxDependsOnConjDependencies) {
                        if (conjDeps.get(rxDependsOn).get(cursorConjDep) == Pulse.HIGH) {
                            System.out.println(cursorConjDep + " cycle=" + cycle);
                            Integer value = matchingCycles.get(cursorConjDep);
                            if (value != null && (cycle - value) > 0) {
                                System.out.println(cycle - value);
                                depsMatchingCyclesDiffs.put(cursorConjDep, cycle - value);
                                if (depsMatchingCyclesDiffs.size() == rxDependsOnConjDependencies.size()) {
                                    shouldContinue = false;
                                    break;
                                }
                            }
                            matchingCycles.put(cursorConjDep, cycle);
                        }
                    }
        ...
        for (Integer value : depsMatchingCyclesDiffs.values()) {
            res = MathUtils.lcm(res, BigInteger.valueOf(value));
        }
